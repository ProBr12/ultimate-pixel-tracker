...
def send_capi_event(event_name, event_id, fbc=None, fbclid=None,
    ...
    logger.info(f"Sending CAPI event: {event_name} | event_id: {event_id} | fbc: {fbc} | fbclid: {fbclid} | email: {email} | phone: {phone}")
...
def order_created():
    ...
    billing = order.get("billing_address") or order.get("shipping_address") or {}
    ...
    logger.info(f"Customer: {email} | {phone} | {city} | {postcode} | {country}")
...
def track_event():
    body = request.json
    logger.info(f"Browser event received: {body.get('event_name')} | fbc: {body.get('fbc')} | fbclid: {body.get('fbclid')}")
